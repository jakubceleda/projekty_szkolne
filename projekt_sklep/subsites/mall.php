...
<div class="rec-added" style='text-align: center;'>
    <img src="../graphics/4m.png" alt="Dodane" style="height:60px; width:150px;">
</div>
...
<div style="width:100%;height:2px;background-color:black;margin:15px 0 15px 0;"></div>
    ...
    <div class="flex-row" style="display:flex; flex-wrap:wrap; margin-bottom: 120px; padding: 0 0 0 4%">
        <?php 
            $sql = "SELECT * FROM products WHERE prod_type LIKE 'm%'";
            $result = $conn->query($sql);

            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    echo "<div style='width:21.5%; height: 450px; background-color:white;border: 1px solid black; margin: 25px 1% 5px 1%; text-align: center;'>";
